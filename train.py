...
def deblur(model, img_tensor, device):
    # Get original image dimensions [Batch, Channels, Height, Width]
    _, _, h, w = img_tensor.shape
    
    # The 4-stage architecture requires dimensions divisible by 8
    factor = 8
    ...
    # Crop the padded pixels back out to restore original dimensions
    if pad_h > 0 or pad_w > 0:
        output = output[:, :, :h, :w]
